...
class ReduceMACTree8 extends Module with HWParameters{
    val io = IO(new Bundle{
        ...
        val CResult = DecoupledIO(UInt(ResultWidth.W))
...
class ReducePE extends Module with HWParameters{
    val io = IO(new Bundle{
        ...
        val ReduceC = DecoupledIO(UInt(ResultWidth.W))
        val ConfigInfo = Flipped(Valid(new ConfigInfoIO))
    })

//    printf(p"PE A2PE ${io.A2PE.valid} ${io.A2PE.ready} Bin ${io.B2PE.valid} ${io.B2PE.ready} Cout ${io.PE2C.ready} ${io.PE2C.valid}\n")

//数据类型
    val dataType = RegInit(ElementDataType.DataTypeUInt8)
    ...
    ReduceMAC8.io.CResult <> io.ReduceC
    ReduceMAC8.io.Valid = io.ConfigInfo.valid && io.ConfigInfo.datatype.bits === ElementDataType.DataTypeUInt8
