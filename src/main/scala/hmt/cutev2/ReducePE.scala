...
class ReduceMACTree8(id:Int) extends Module with HWParameters{
    ...
    val ReduceTreeData = WireInit(VecInit(Seq.fill(ReduceTreeDepth +1)(VecInit(Seq.fill(ReduceWidth/8)(0.U(ReduceTreeWidth.W))))))
    val ReduceTreeRegData = RegInit(VecInit(Seq.fill(ReduceTreeDepth +1)(VecInit(Seq.fill(ReduceWidth/8)(0.U(ReduceTreeWidth.W))))))
    ...
    for(i <- 0 until ReduceTreeDepth){
        for(j <- 0 until ReduceLevelElement(i)){
            ...
            if(i==3)
            {
                ReduceTreeRegData(i) := ReduceTreeData(i)
                ReduceTreeRegValid(i) := ReduceTreeValid(i)
            }
            if(i==2)
            {
        ...
        when(ReduceTreeValid(i)){
            if(id == 0){
                ...
                val DebugReduceTreeData_i = Wire(Vec(ReduceWidth/8,UInt(DebugReduceTreeWidth.W)))
                DebugReduceTreeData_i := 0.U.asTypeOf(DebugReduceTreeData_i)
                for(j <- 0 until ReduceLevelElement(i))
                {
    ...
    val CExternalReduceAddChain = RegInit(VecInit(Seq.fill(ExternalReduceSize)(0.U(ResultWidth.W))))
    val CExternalReduceAddChainValid = RegInit(VecInit(Seq.fill(ExternalReduceSize)(false.B)))

    //CAdd的FIFO
    val CDelayFIFO = RegInit(VecInit(Seq.fill(CDelayFIFODepth)(0.U(ResultWidth.W))))
    ...
    when(io.CAdd.valid){
        when(!CDelayFIFOFull){
            CDelayFIFO(CDelayFIFOHead) := io.CAdd.bits
            if(id == 0)
            {
    ...
    for(i <- 1 until ExternalReduceSize){
        CExternalReduceAddChainValid(i) := false.B
        when(CExternalReduceAddChainValid(i-1)){
            CExternalReduceAddChain(i) := CExternalReduceAddChain(i-1) + ReduceTreeData(0)(0)
            CExternalReduceAddChainValid(i) := true.B
            if(id==0)
            {
    ...
    when(CExternalReduceAddChainValid(ExternalReduceSize-1)){
        io.DResult.bits := CExternalReduceAddChain(ExternalReduceSize-1)
        io.DResult.valid := true.B
        if(id==0)
        {
