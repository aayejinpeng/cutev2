...
class AScratchpad extends Module with HWParameters{
    ...
    val sram_banks = (0 until AScratchpadNBanks) map { i =>
        ...
        {
            //输出读的信息
            printf("[ASPD_Read]Bank(%d): debug_s1_bank_addr = %d ,s1_bank_read_data = %x\n", i.U, debug_s1_bank_addr(0), s1_bank_read_data)
        }
        ...
        when(write_go && s0_bank_write_valid && i.U === MemoryLoaderBankId){
            bank.write(s0_bank_write_addr, s0_bank_write_data)
            printf("[ASPD_Write]Bank(%d): s0_bank_write_addr = %d ,s0_bank_write_data = %x\n", i.U, s0_bank_write_addr, s0_bank_write_data)
        }
