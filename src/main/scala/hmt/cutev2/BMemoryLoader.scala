...
class BMemoryLoader(implicit p: Parameters) extends Module with HWParameters{
    ...
    when(io.ConfigInfo.valid){
        when(io.ConfigInfo.bits.taskType === CUTETaskType.TaskTypeMatrixMul){
            when(state === s_idle){
                ...
                Tensor_B_BaseVaddr := io.ConfigInfo.bits.ApplicationTensor_B.ApplicationTensor_B_BaseVaddr
                MemoryOrder_LoadConfig := io.ConfigInfo.bits.ApplicationTensor_B.MemoryOrder
                Tensor_Block_BaseAddr := io.ConfigInfo.bits.ApplicationTensor_B.BlockTensor_B_BaseVaddr
                Conherent := io.ConfigInfo.bits.ApplicationTensor_B.Conherent
    ...
    when(memoryload_state === s_load_init){
        ...
        when(MemoryOrder_LoadConfig === MemoryOrderType.OrderType_Mb_Kb)
        {
            ...
            Request.bits.RequestVirtualAddr := Tensor_Block_BaseAddr + (CurrentLoaded_BlockTensor_N * Tensor_K.U + CurrentLoaded_BlockTensor_K) * (ReduceWidthByte.U)
            ...
            when(MemoryOrder_LoadConfig === MemoryOrderType.OrderType_Nb_Kb && CurrentLoaded_BlockTensor_N === Tensor_N.U){
                Request.valid := false.B
            }
            ...
            when(Request.ready && sourceId.valid){//符合条件的话，这条访存请求一定会被发出
                ...
                // SoureceIdSearchTable(sourceId.bits).ScratchpadBankId := CurrentLoaded_BlockTensor_N % BScratchpadNBanks.U
                // SoureceIdSearchTable(sourceId.bits).ScratchpadAddr := ((CurrentLoaded_BlockTensor_N / BScratchpadNBanks.U) * Tensor_K.U) + CurrentLoaded_BlockTensor_K
                when(MemoryOrder_LoadConfig === MemoryOrderType.OrderType_Nb_Kb){
